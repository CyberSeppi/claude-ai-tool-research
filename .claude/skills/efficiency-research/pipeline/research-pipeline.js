...
const found = (await parallel(topics.map((t, i) => () =>
  agent(
    ...
    `MANDATORY: Before naming any repos, use the WebSearch tool to run at least 3 fresh searches. Suggested queries: (1) the core mechanism keywords from this topic + "GitHub 2026", (2) "best GitHub repos" + the topic's primary use-case, (3) "new OR trending" + the topic's tool-type + " Claude Code". Collect candidate names from live search results — DO NOT rely on training-data recall as the primary discovery method; the model's training cutoff is January 2026 and many high-star post-cutoff repos (e.g. rtk-ai/rtk, headroomlabs-ai/headroom — both released after the cutoff yet 50k+ stars) are invisible without fresh search. Also issue a GitHub-native search like "topic:claude-code+topic:token-optimization" when applicable.\n` +
